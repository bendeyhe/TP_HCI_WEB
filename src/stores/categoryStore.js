...
export const useCatgoryStore = defineStore("category", {
    state: () => ({
        categories: ref([]),
        apiEndpoints: {
            getCategories: {path: `${CAT_B_URL}/categories`, requiresAuth: true, method: 'GET'},
            addCategory: {path: `${CAT_B_URL}/categories`, requiresAuth: true, method: 'POST'},
            getCategory: (categoryId) => ({path: `${CAT_B_URL}/categories/${categoryId}`, requiresAuth: true, method: 'GET'}),
            changeCategory: (categoryId) => ({path: `${CAT_B_URL}/categories/${categoryId}`, requiresAuth: true, method: 'PUT'}),
            deleteCategory: (categoryId) => ({path: `${CAT_B_URL}/categories/${categoryId}`, requiresAuth: true, method: 'DELETE'})
        },
